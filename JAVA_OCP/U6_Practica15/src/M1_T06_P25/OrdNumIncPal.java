...
public class OrdNumIncPal extends AlgoritmoOrdenacion {
	...
	public static int comparadorCarta(Carta c1, Carta c2) {
		...
		if((ordenarNumInc(transcript(c1.getNumero()), transcript(c2.getNumero()))<0)) {
			//El palo de c1 es mayor que el palo c2 => Intercambiamos
			return -1;
		}else {
			if((ordenarNumInc(transcript(c1.getNumero()), transcript(c2.getNumero()))<=0) && ((ordenarPalo(c1.getPalo().substring(0,1),c2.getPalo().substring(0,1)))<0)) {
